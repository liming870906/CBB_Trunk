...
public class MP3Recorder extends BaseRecorder {
                ...
                while (mIsRecording) {
                    ...
                    if (readSize == AudioRecord.ERROR_INVALID_OPERATION ||
                        ...
                        if (readSize > 0) {
                            ...
                            mEncodeThread.addTask(mPCMBuffer, readSize);//将录音数据添加到编码线程中
    ...
    private void sendData(short[] shorts, int readSize) {
        if (dataList != null) {
            int length = readSize / 287;
            short resultMax = 0, resultMin = 0;
            for (short i = 0/*, k = 0*/; i < readSize; /*i++, */i += 287) {
                resultMax = shorts[i];

                if (resultMax > 10000 || resultMax < -10000) {
                    resultMax = 10000;
                }
//                for (short j = k, max = 0, min = 1000; j < k + 356; j++) {
//
//                    if (shorts[j] > max) {
//                        max = shorts[j];
//                        resultMax = max;
