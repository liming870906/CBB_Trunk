...
//        }
    ...
    public void play() throws IOException {
        System.out.println("doStop = " + doStop + " seekOffsetFlag = " + seekOffsetFlag + " mState: " + mState);
        if (!seekOffsetFlag && doStop && dataList != null) {
...
//        myTimer.scheduleAtFixedRate(myTimerTask, 0, 1000); //(timertask,delay,period)
    ...
    class DelegateHandler extends Handler {
        ...
        public void onRadioPlayerUpdate(MP3RadioStreamPlayer player){
            if (MP3RadioStreamPlayer.this.mDelegate != null) {
                MP3RadioStreamPlayer.this.mDelegate.onRadioPlayerUpdate(player);
            }
        }
    }
    ...
    private void decodeLoop() {
        ...
        Log.i(LOG_TAG, "channelConfiguration " + channelConfiguration);
        ...
        //开始play，等待write发出声音
        audioTrack.play();
        audioTrack.setPlaybackRate(defaultSpeed == 0 ? 48000 : 72000);
...
//            Log.i("info","Time=====>Mp3RadioStream_start:"+ SystemClock.currentThreadTimeMillis());
            //跳出循环
            if(doStop){
                break;
            }
...
//                    Log.i("info","Time=====>Mp3RadioStream_stop:"+ SystemClock.currentThreadTimeMillis());
//                    calculateRealVolume(music, music.length);
                    ...
                    mDelegateHandler.onRadioPlayerUpdate(MP3RadioStreamPlayer.this);
                    hadPlay = true;
                }
                //释放
    ...
    private void sendData(short[] shorts, int readSize) {
        if (dataList != null) {
            ...
            for (short i = 0/*, k = 0*/; i < readSize; /*i++, */i += 382) {
                resultMax = shorts[i];
    ...
    public void seekTo(long time, boolean pause) {
        this.pause = pause;
    ...
    public boolean getSeekOffsetFlag() {
        return seekOffsetFlag;
    }

    public void SetSoundSpeed(int value) {
        setDefaultSpeed(value);
        audioTrack.setPlaybackRate(defaultSpeed == 0 ? 48000 : 72000);
    }

    public void setDefaultSpeed(int value){
        defaultSpeed = value;
    }
}
